...
<p>Initial-value problems for ordinary and partial differential equations often possess conservation laws (e.g. for energy) and dissipation inequalities (e.g. for entropy). Standard discretisation methods, such as many Runge-Kutta and multistep schemes, usually do not preserve these structural properties. In this talk, we present a general strategy for simultaneously enforcing multiple conservation laws and dissipation inequalities in the numerical solution of initial-value problems. The key idea is to systematically modify a starting discretisation so that the continuous arguments establishing conservation and dissipation carry over to the discrete level. Each conservation law or dissipation inequality is represented by an associated test function; we introduce auxiliary variables for the projections of these test functions onto a discrete test set, and modify the equation to use these new variables.</p>

<p>We demonstrate this strategy by applying it to Hamiltonian ordinary and partial differential equations, geometric flows, the incompressible and compressible Navier-Stokes equations, and a viscoelastic model. In each case, the strategy yields discretisations with new and useful properties. In several cases, preserving conservation and/or dissipation laws appears to offer considerable qualitative advantages over preserving other structures, such as symplecticity.</p>
...
<p>Building on the seminal work of Schöberl, Olshanskii, and Benzi, in this talk we present a novel preconditioner for the Newton linearisation of the stationary Navier--Stokes equations in three dimensions that achieves both Reynolds robustness and optimal complexity in dof count at low orders. The exact details of the preconditioner vary with discretisation, but the general theme is to combine augmented Lagrangian stabilisation, a custom multigrid prolongation operator involving local solves on coarse cells, and an additive patchwise relaxation on each level that captures the kernel of the divergence operator.</p>
